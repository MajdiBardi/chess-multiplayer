      ...
      <div class="auth-card">
        ...
        <form (ngSubmit)="onSubmit()" #f="ngForm">
          ...
          <div class="field">
            ...
            <input type="password" name="password" [(ngModel)]="password" required minlength="4" #p="ngModel" />
            @if (p.invalid && p.touched) {
              <span class="error">Minimum 4 caractères</span>
            }
          </div>
          @if (error) {
            <p class="error">{{ error }}</p>
          }
          <button type="submit" class="primary" [disabled]="f.invalid || loading">Créer le compte</button>
        </form>
...
export class RegisterComponent {
  ...
  onSubmit(): void {
    ...
    this.auth.register(this.username, this.password).subscribe({
      ...
      error: (err) => {
        this.loading = false;
        this.error = err.error?.message || 'Ce nom d\'utilisateur est déjà pris ou erreur serveur';
      },
